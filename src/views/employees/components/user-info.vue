...
import { getEmployeeDetail, updateEmployee, getPersonalDetail, updatePersonal } from '@/api/employees'
...
export default {
  ...
  methods: {
    async loadPersonalDetail() {
      ...
      //个人信息里面，如果有staffPhoto这个属性的话，需要回显头像
      const staffPhoto = this.formData.staffPhoto
      //避免了 staffPhoto
      if (this.formData.staffPhoto) {
        this.$refs.myStaff.fileList = [{ url: this.formData.staffPhoto, upload: true }]
      }
    },
    ...
    async saveUser() {
      ...
      //保存更新的按钮，加一个disabled属性，这个属性为true的条件 
      //computed: { saveBtnDisabled() {return fileList.some(item => !item.upload)}}
      if (fileList.some(item => !item.upload)) {
        return this.$message.warning('图片正在上传中，请稍后')
      }
      //({...this.userInfo, staffPhoto:'最新的图片地址'}) 对象里面覆盖原有的属性
      //合并对象操作 object.assign({原对象} ， this.userInfo,{ staffPhoto:fileList[0].url?fileList[0].url})
      await updateEmployee({ ...this.userInfo, staffPhoto: fileList[0].url ? fileList[0].url : ' ' })
